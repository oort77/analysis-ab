...
df['revenue_weight'] = df['revenue'].map(df['revenue'].value_counts(normalize=True).to_dict())
...
db_score = alt.Chart(df_plot).mark_line(point=True).encode(
...
adr_chart = alt.Chart(df_km).transform_density(
    # density='adr',
    density='revenue',
    # bandwidth=10,
    groupby=['cluster'],
    ...
    counts = True,
    steps=50
).mark_area(opacity=0.75).encode( # area
    alt.X('value:Q'),
